...
def do_define_form(expressions, env):
    ...
    #Pair(x, Pair(b, nil)) where b is call of (+ 1 2)
    if scheme_symbolp(signature):
        ...
        b = expressions.rest
        b_value = b.first
        expression_value = scheme_eval(b_value, env, _=None)
        Frame.define(env, signature, expression_value)
       ...
       # validate_form(expressions, 2, 2)  # Checks that expressions is a list of length exactly 2
        # BEGIN PROBLEM 4
       # if len(expressions.rest) == 1: # we know its a num or like other var
       #     Frame.define(expressions, expressions.first, expressions.rest)

       # elifpython3 ok -q 05 -u len(expressions.rest)  > 1: # we know its a call expresions
       #     b = scheme_eval(expressions.rest, env, _=None)
        #    Frame.define(expressions.first, b)
       ...
       # return signature
        ...
        #signiture.first = lambda name
        #body = expressions.rest
        #formals = signirure.rest
        #lambda_instance = do_lambda_form(expressions, env)
        #signature.first = lambda_instance
        validate_form(expressions, 2)
        formals = signature.rest
        validate_formals(formals)

        lambda_instance = LambdaProcedure(signature.rest, expressions.rest, env)
        env.bindings[signature.first] = lambda_instance
...
def do_and_form(expressions, env):
    ...
    copy = scheme_eval(expressions.first, env)
    if not expressions.rest:
        return copy

    elif is_scheme_true(copy):

        return do_and_form(expressions.rest, env)

    elif not is_scheme_true(copy):
        return copy
...
def make_let_frame(bindings, env):
    ...
    copy = bindings
    formals = bindings.first.first
    first = copy.first.rest.first


    while copy:
        validate_form(copy.first, 2, 2)
        val1 = scheme_eval(copy.first.rest.first, env, _=None)
        names = Pair(copy.first.first, names)
